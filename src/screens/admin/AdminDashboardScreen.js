...
export default function AdminDashboardScreen({ navigation }) {
  ...
  return (
    <View style={styles.root}>
      {/* Dark sidebar strip on left for desktop feel */}
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsGrid}>
          <StatCard label="Total Riders"   value={stats.totalUsers}    icon="people"           accent="#3B82F6" delta="+3 today" />
        ...
        <View style={styles.section}>
            ...
            {dummyRideRequests.map(r => (
              <View key={r.id} style={styles.rideRow}>
                <View style={styles.rideTypeIcon}>
                  <Text style={{ fontSize: 16 }}>{r.rideType === 'Auto' ? '🛺' : r.rideType === 'Cab Economy' ? '🚗' : '🚙'}</Text>
                </View>
                <View style={{ flex: 1 }}>
          ...
          <View style={styles.actionGrid}>
            {[
              { screen: 'UsersList',   icon: 'people',   label: 'Users',   count: stats.totalUsers,   accent: '#3B82F6' },
              { screen: 'DriversList', icon: 'bicycle',  label: 'Drivers', count: stats.totalDrivers, accent: colors.bike },
            ].map(a => (
              <TouchableOpacity
                key={a.screen}
              ...
              >
                ...
                <Text style={styles.actionCount}>{a.count} total</Text>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingTop: 56, paddingHorizontal: 18, paddingBottom: 24,
  ...
  brand: { fontSize: 18, fontWeight: '900', color: colors.black },
  ...
  revenueValue: { fontSize: 34, fontWeight: '900', color: colors.black, letterSpacing: -1 },
