      ...
      <View style={[styles.card, shadow.card]}>
        <View style={styles.riderRow}>
          <View style={styles.avatar}>
            <Text style={{ fontWeight: '800', fontSize: 18 }}>{request.riderName.split(' ').map(n => n[0]).join('')}</Text>
          </View>
          <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  timerText: { color: colors.white, fontWeight: '800', fontSize: 13 },
  ...
  fare: { fontSize: 20, fontWeight: '800', color: colors.black },
  ...
  metaItem: { gap: 2 },
  swipeWrap: { marginTop: 'auto', marginBottom: 30, alignItems: 'center', gap: 14 },
