...
export default function SwipeButton({ label = 'Swipe to accept', onSwipeSuccess, color = colors.success }) {
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: (_, gesture) => {
        if (gesture.dx >= SWIPE_RANGE * 0.85 && !completed.current) {
          ...
          Animated.timing(pan, { toValue: SWIPE_RANGE, duration: 120, useNativeDriver: false })
            .start(() => onSwipeSuccess && onSwipeSuccess());
        } else {
  ...
  return (
    <View style={[styles.track, { width: TRACK_WIDTH, backgroundColor: color + '22', borderColor: color }]}>
...
const styles = StyleSheet.create({
  ...
  trackLabel: { fontSize: 15, fontWeight: '700', position: 'absolute', alignSelf: 'center' },
  thumb: {
    position: 'absolute',
    left: 4,
    width: THUMB_SIZE,
