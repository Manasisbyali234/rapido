...
import { logout, getCurrentUser } from '../../utils/authStore';
  ...
  if (error) {
    return (
      <View style={styles.centered}>
        ...
        <TouchableOpacity style={[styles.retryBtn, { backgroundColor: colors.dangerBg, marginTop: 8 }]} onPress={handleLogout}>
          <Text style={[styles.retryText, { color: colors.danger }]}>Log Out</Text>
        </TouchableOpacity>
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  confirmText: { fontSize: 14, fontWeight: '600', color: colors.black, marginBottom: 12, textAlign: 'center' },
  confirmBtns: { flexDirection: 'row', gap: 10 },
  confirmCancel: {
    flex: 1, borderWidth: 1.5, borderColor: colors.border,
    borderRadius: radius.pill, paddingVertical: 12, alignItems: 'center',
  },
  confirmCancelText: { fontSize: 14, fontWeight: '700', color: colors.black },
  confirmLogout: {
