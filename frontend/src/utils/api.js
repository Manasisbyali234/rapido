...
export const BASE_URL = 'http://192.168.68.119:3000';
...
export const registerCaptain = (name, phone, email, vehicle, vehicleNumber, licenseNumber) => request('/auth/captain/register', { method: 'POST', body: JSON.stringify({ name, phone, email, vehicle, vehicleNumber, licenseNumber }) });
export const loginUser       = (phone)              => request('/auth/user/login',          { method: 'POST', body: JSON.stringify({ phone }) });
export const loginCaptain    = (phone)              => request('/auth/captain/login',       { method: 'POST', body: JSON.stringify({ phone }) });
...
export const adminGetCaptains        = (search) => request(`/admin/captains${search ? `?search=${encodeURIComponent(search)}` : ''}`);
export const adminGetPendingCaptains = ()       => request('/admin/captains/pending');
export const adminGetRides           = (params) => request(`/admin/rides${params ? `?${params}` : ''}`);
export const adminApproveCaptain     = (id)     => request(`/admin/captains/${id}/approve`,        { method: 'PATCH' });
export const adminRejectCaptain      = (id)     => request(`/admin/captains/${id}/reject`,         { method: 'PATCH' });
export const adminSuspendCaptain     = (id)     => request(`/admin/captains/${id}/toggle-suspend`, { method: 'PATCH' });
